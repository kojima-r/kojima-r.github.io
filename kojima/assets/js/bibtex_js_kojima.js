...
function BibtexDisplay() {
  ...
  this.count=0;
  this.displayBibtex = function(input, output,header) {
    // parse bibtex input
    var b = new BibtexParser();
    b.setInput(input);
    try {
    ...
    for (var entryKey in entries) {
      ...
      var tpl = $(".bibtex_template").clone().removeClass('bibtex_template');
      //
      var bib_id=tpl.find(".bib_id");
      
      if(bib_id.size()>0){
        bib_id.html(header+id);
      }
  ...
  if(str){
    ...
    for (var i=0;i<arr.length;i++) {
      ...
      for (var j=0;j<names.length;j++) {
        ...
        if(j>0&&names[j].length>1&&isAlphabetName(names[j])){
          // for middle name
          ns=names[j].split(" ")
      ...
      if(names.length>1){
        ...
        // "Ryosuke Kojima"
        if(names[0].length>1 && isAlphabetName(names[0][0])){
          ns=names[0].split(" ")
          for (var k=0;k<ns.length-1;k++) {
            ns[k]=ns[k][0]+".";
          }
          names[0]=ns.join(" ");
        }
      ...
      //console.log(names)
      auther_str.push(names.join(""));
    }
    return auther_str.join(",  ");
  }else{
    return false;
  }
}
function bibtex_js_draw() {
  ...
  (new BibtexDisplay()).displayBibtex($("#bibtex_journal").val(), $("#bibtex_display_journal"),"t");
//  (new BibtexDisplay()).displayBibtex($("#bibtex_journal2").val(), $("#bibtex_display_journal2"),"T");
  (new BibtexDisplay()).displayBibtex($("#bibtex_conf").val(), $("#bibtex_display_conf"),"c");
